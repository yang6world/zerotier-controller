...
function install() {
  ...
  docker run -d   --network host   --name zerotier   --restart unless-stopped -e ZU_CONTROLLER_ENDPOINT=http://127.0.0.1:9993/   -e ZU_SECURE_HEADERS=false   -e ZU_DATAPATH=/app/backend/data/db.json   -e ZU_DEFAULT_USERNAME=$user_set   -e ZU_DEFAULT_PASSWORD=$user_password   -p 4000:4000/tcp  -p $port:$port -p $port:$port/udp  $imageName
